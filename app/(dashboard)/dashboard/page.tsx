...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const newStats = isAllLocales
      ? mockStats
      : (mockStatsByLocal[selectedLocal?.id || "1"] ?? mockStats);
...
function StatCard({
  ...
  return (
    <div className="relative bg-white rounded-xl shadow-sm border border-slate-200 p-6 overflow-hidden group hover:shadow-md transition-all duration-200">
      ...
      <div
        className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${gradient} opacity-5 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform duration-300`}
      />
      ...
      <div className="relative flex items-start justify-between">
        ...
        <div
          className={`stat-icon bg-gradient-to-br ${gradient} text-white shadow-lg`}
        >
          {icon}
        </div>
      </div>
...
function ProductItem({
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all duration-200 group">
      <div
        className={`w-8 h-8 ${rankColors[rank - 1]} rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-sm`}
      >
        {rank}
      </div>
...
function AlertItem({
  ...
  return (
    <div
      className={`flex items-start gap-3 p-4 rounded-xl border ${config.bg} ${config.border} hover:shadow-sm transition-shadow duration-200`}
    >
      <div
        className={`p-2 rounded-lg ${config.bg} ring-1 ring-inset ${config.border}`}
      >
        <AlertTriangle size={18} className={config.icon} />
      </div>
      <p className={`text-sm flex-1 ${config.text} leading-relaxed`}>
        {message}
      </p>
    </div>
  );
